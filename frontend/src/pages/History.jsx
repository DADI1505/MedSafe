...
export default function History({ initialEntry }) {
  const { data: history, isLoading, isError } = useAuditHistory()

  // Si une entrée précise a été sélectionnée depuis la barre latérale,
  // on l'affiche seule et en avant, plutôt que toute la liste
  const displayList = initialEntry ? [initialEntry] : history

  return (
    <div className="mx-auto max-w-2xl px-4 py-8">
      <h1 className="mb-6 flex items-center gap-2 text-2xl font-bold text-brand">
        <Clock className="h-6 w-6" aria-hidden="true" />
        {initialEntry ? "Question sélectionnée" : "Historique de vos questions"}
      </h1>

      {isLoading && <p className="text-muted">Chargement de votre historique…</p>}
      ...
      {displayList && displayList.length > 0 && (
        <div className="space-y-4">
          {displayList.map((entry) => (
            <article key={entry.id} className="rounded-card border border-line bg-surface p-5 shadow-sm">
              <div className="mb-3 flex items-center justify-between gap-3">
                <p className="font-medium text-brand">{entry.query_text}</p>
                <RiskBadge level={entry.risk_level} />
              </div>
              ...
              {entry.generated_response && (
                <div className="text-sm">{parseMarkdown(entry.generated_response)}</div>
              )}
